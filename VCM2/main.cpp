...
public:
    void handleRequest(HTTPServerRequest & request, HTTPServerResponse & response) override {
        string path = string("c:/workspace/dev/products/VCM/main_product/VCM UI") + request.getURI();
        if (request.getURI() == "/") {
            path += "index.html";
        }
        try {
...
//gkocot 
    ...
    HTTPRequestHandler * createRequestHandler(const HTTPServerRequest & request) override {
        //gkocot std::cout << request.getMethod() << std::endl;
        //gkocot std::cout << request.getURI() << std::endl;
